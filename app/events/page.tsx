...
// ── Simple ML: find similar events by category ──────────────────────────────
...
export default function EventsPage() {
  ...
  const [form, setForm] = useState({
    title: "", date: "", location: "", category: "Other",
    description: "", expectedAttendance: 20,
  });
  ...
  const toggleStatus = (id: string) => {
    setEvents(events.map((e) =>
      e.id === id ? { ...e, status: e.status === "upcoming" ? "done" : "upcoming" } : e
    ));
  };
    ...
    <main className="min-h-screen bg-gray-50 p-6">
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                id="attendance" type="number" min={1}
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        ...
        <select
          ...
          className="border border-gray-300 rounded-full px-4 py-1.5 text-sm text-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {filtered.map((event) => (
            <div
              key={event.id}
              className={`bg-white rounded-xl shadow p-5 flex flex-col justify-between cursor-pointer border-2 transition-colors ${
                selectedEvent?.id === event.id ? "border-blue-500" : "border-transparent hover:border-blue-200"
              }`}
            ...
            >
              ...
              <div className="flex gap-2 mt-4" onClick={(e) => e.stopPropagation()}>
                <button
                  ...
                  aria-label={`Mark ${event.title} as ${event.status === "upcoming" ? "done" : "upcoming"}`}
                  className="flex-1 bg-yellow-400 hover:bg-yellow-500 text-white text-sm py-1.5 rounded-lg transition-colors"
                >
                ...
                <button
                  ...
                  className="flex-1 bg-red-500 hover:bg-red-600 text-white text-sm py-1.5 rounded-lg transition-colors"
                >
                  Delete
                </button>
              </div>
      ...
      {/* ML: Similar Events Panel */}
      {selectedEvent && (
        <div className="mt-10 bg-white rounded-xl shadow p-6">
          <h2 className="text-xl font-bold text-gray-800 mb-1">
            🤖 Similar Events to "{selectedEvent.title}"
          </h2>
          <p className="text-sm text-gray-500 mb-4">
            Recommended based on category matching — <span className="font-medium">{selectedEvent.category}</span>
          </p>
